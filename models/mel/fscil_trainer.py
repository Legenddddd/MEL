        ...
        for session in range(args.start_session, args.sessions):
            ...
            # incremental learning sessions
                ...
                for batch in trainloader:
                    data0, label = [_.cuda() for _ in batch]
                    data = self.model.module.encode1(data0)

                with torch.no_grad():

                    _, d = data.shape
                    query = self.model.module.attention.data.view(1, d)

                    attn_scores = torch.matmul(query, data.T) / d ** 0.5
                    attn_weights = F.softmax(attn_scores, dim=1)

                    fused_vector = torch.matmul(attn_weights, data)
                    test_class = args.base_class + session * args.way
                    self.model.module.attention.data = (args.way / test_class) * fused_vector.view(1, d, 1, 1) + (
                            (test_class - args.way) / test_class) * self.model.module.attention.data


                self.model.module.update_fc(trainloader, np.unique(train_set.targets), criterion_Cross, session)
